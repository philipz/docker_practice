...
> **版本說明：** 本節示例使用 etcd v3.5.29。請存取 [etcd 官方釋出頁](https://github.com/etcd-io/etcd/releases) 取得最新版本資訊。

下面我們使用 [Docker Compose](../compose/README.md) 模擬啟動一個 3 節點的 `etcd` 集群。
...
```
使用 `docker compose up` 啟動集群之後使用 `docker exec` 命令登入到任一節點測試 `etcd` 集群。
